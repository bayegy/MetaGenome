...
{R_path} {bayegy_home}/Function_DunnTest.r -i  {abundance_table} -m  {mapping_file} -c  {category} -o  {out_dir}4-SignificanceAnalysis/DunnTest -p {prefix};
            ...
            with OSEnv(pythonpath=self.lefse_pylib_home, path=self.lefse_py_home, r_libs=self.lefse_rlib_home):
                self.system("""
{bayegy_home}/mod_format_input.py {out_dir}4-SignificanceAnalysis/LEfSe/{prefix}{category}_lefse.txt {base}2.lefseinput.txt -c 2 -u 1 -o 1000000; {bayegy_home}/mod_run_lefse.py {base}2.lefseinput.txt {base}2.LDA.txt -l 2 -a 0.03 -w 0.03;
...
#{bayegy_home}/mod_lefse-plot_cladogram.py {base}2.LDA.txt  --map {mapping_file} --category {category} --dpi 300 {base}2.cladogram.pdf --clade_sep 1.8 --format pdf --right_space_prop 0.45 --label_font_size 10;
#{bayegy_home}/mod_format_input.py {out_dir}4-SignificanceAnalysis/LEfSe/{prefix}{category}_lefse.txt {base}4.lefseinput.txt -c 2 -u 1 -o 1000000; run_lefse.py {base}4.lefseinput.txt {base}4.LDA.txt -l 4 -a 0.03 -w 0.03;
...
#{bayegy_home}/mod_lefse-plot_cladogram.py {base}4.LDA.txt --map {mapping_file} --category {category} --dpi 300 {base}4.cladogram.pdf --clade_sep 1.8 --format pdf --right_space_prop 0.45 --label_font_size 10;
                """, base='{}4-SignificanceAnalysis/LEfSe/{}{}_lefse_LDA'.format(self.out_dir, self.prefix, g), category=g)
            # if not exclude == 'all':
