...
MAIN() {
	...
	source deactivate
	source activate qm2
	for key in ${!tax_aa[*]};do mv ${output_dir}/Relative/otu_table.${key}.relative.mat ${output_dir}/Relative/otu_table.${tax_aa[$key]}.relative.txt;done;
	...
	for category_1 in $category_set;
		do echo $category_1;
			Rscript ${SCRIPTPATH}/write_data_for_lefse.R  $otu_table  $mapping_file  $category_1  ${output_dir}/LEfSe/${category_1}_table_for_lefse.txt;
			base="${category_1}_LEfSe"; format_input.py ${output_dir}/LEfSe/${category_1}_table_for_lefse.txt ${output_dir}/LEfSe/${base}.lefseinput.txt -c 2 -u 1 -o 1000000; run_lefse.py ${output_dir}/LEfSe/${base}.lefseinput.txt ${output_dir}/LEfSe/${base}.LDA.txt -l 2;  plot_res.py --left_space 0.3 --dpi 300 ${output_dir}/LEfSe/${base}.LDA.txt ${output_dir}/LEfSe/${base}.png; plot_cladogram.py ${output_dir}/LEfSe/${base}.LDA.txt --dpi 300 ${output_dir}/LEfSe/${base}.cladogram.png --format png --right_space_prop 0.45 --label_font_size 10;
			plot_res.py  --max_feature_len 200 --orientation h --format pdf --left_space 0.3 --dpi 300 ${output_dir}/LEfSe/${base}.LDA.txt ${output_dir}/LEfSe/${base}.pdf; plot_cladogram.py ${output_dir}/LEfSe/${base}.LDA.txt --dpi 300 ${output_dir}/LEfSe/${base}.cladogram.pdf --clade_sep 1.8 --format pdf --right_space_prop 0.45 --label_font_size 10;
		done;
